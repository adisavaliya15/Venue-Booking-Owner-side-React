...
export default function EditDecoration() {
  ...
  const [decorationData, setDecorationData] = useState({
    venueId: location.state.venue.venueId,
    decorId: location.state.decor.decorId,
    price: "",
    image: "",
  });
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(
        "http://localhost:8000/api/owener/editDecoration",
        data,
        {
          headers: {
        ...
        <div className="container-fluid pt-4 px-4">
          <div className="bg-light text-center rounded p-4">
            <div className="d-flex align-items-center justify-content-between mb-4">
              <h6 className="mb-0">Add Decoration</h6>
            </div>
